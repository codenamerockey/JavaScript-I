...
let oldCars = carYears.filter(cars => {
  return cars < 2000;
});
...
let bmw = inventory.filter(specificCar => specificCar.car_make === 'BMW');

let audi = inventory.filter(specificCar => specificCar.car_make === 'Audi');

let BMWAndAudi = JSON.stringify([...bmw, ...audi]);
